...
document.addEventListener('mousemove', (e) => {
  ...
  if (Math.random() < 0.15) createSparkle(mouseX, mouseY);
  applyParallax(e);
});
...
setInterval(createFloatingEl, 1800);
for (let i = 0; i < 6; i++) setTimeout(createFloatingEl, i * 400);
...
setInterval(triggerHeartRain, 35000);
...
function triggerFinalExplosion() {
  ...
  for (let i = 0; i < 25; i++) {
    setTimeout(() => {
      ...
      h.style.transition = 'all 2s cubic-bezier(.25,.46,.45,.94)';
      h.style.opacity = '1';
      c.appendChild(h);
      const angle = Math.random() * Math.PI * 2;
      const dist = 80 + Math.random() * 200;
      requestAnimationFrame(() => {
        h.style.transform = `translate(${Math.cos(angle)*dist}px,${Math.sin(angle)*dist}px) rotate(${Math.random()*360}deg) scale(0.3)`;
        h.style.opacity = '0';
      });
